...
function initTutor(){
  ...
  $("tutorExplainBtn").onclick=()=>askTutor("Spiegami il capitolo corrente in modo chiaro, con parole semplici e senza dare risposte ai quiz.");
  $("tutorSimpleBtn").onclick=()=>askTutor("Rispiegami questo capitolo come se partissi da zero. Usa frasi brevi e un esempio facile.");
  $("tutorExampleBtn").onclick=()=>askTutor("Fammi esempi pratici su questo capitolo, possibilmente legati alla scuola agraria o alla vita quotidiana.");
  $("tutorOralBtn").onclick=()=>askTutor("Fammi 5 domande orali di allenamento sul capitolo, poi dammi un metodo per rispondere. Non dare risposte ai quiz.");
  $("tutorAskBtn").onclick=()=>{
    const q=$("tutorQuestion").value.trim();
    if(!q){toast("Scrivi una domanda per il tutor");return;}
    askTutor(q);
    $("tutorQuestion").value="";
  };
  $("tutorClearBtn").onclick=()=>{$("tutorChat").innerHTML="";renderTutorIntro();};
}
function renderTutorIntro(){
  const box=$("tutorChat");
  if(!box || box.innerHTML.trim()) return;
  const l=lesson();
  box.innerHTML=`<div class="tutor-msg tutor-ai"><b>Tutor pronto.</b>\nSto lavorando sul capitolo: ${escapeHtml(l.title)}.\nPosso spiegare, semplificare, fare esempi e preparare all'orale. Non posso dare risposte ai quiz o alla simulazione.</div>`;
}
function isTutorBlockedRequest(text){
  const t=String(text||"").toLowerCase();
  const blocked=["risposta esatta","risposta corretta","che risposta","quale opzione","scegli","dimmi la risposta","soluzione del quiz","soluzione esame","a b c","abc","opzione a","opzione b","opzione c","test","simulazione"];
  return blocked.some(x=>t.includes(x));
}
async function askTutor(userText){
  const chat=$("tutorChat");
  if(!chat) return;
  if(isTutorBlockedRequest(userText)){
    chat.innerHTML+=`<div class="tutor-msg tutor-user"><b>Arianna:</b> ${escapeHtml(userText)}</div>`;
    chat.innerHTML+=`<div class="tutor-msg tutor-blocked"><b>Tutor:</b> Non posso dare risposte a quiz o simulazioni. Posso però spiegarti l'argomento, farti un esempio simile o allenarti all'orale senza indicare l'opzione corretta.</div>`;
    chat.scrollTop=chat.scrollHeight;
    return;
  }
  const l=lesson();
  chat.innerHTML+=`<div class="tutor-msg tutor-user"><b>Arianna:</b> ${escapeHtml(userText)}</div>`;
  chat.innerHTML+=`<div class="tutor-msg tutor-ai" id="tutorLoading"><b>Tutor:</b> Sto preparando una spiegazione...</div>`;
  chat.scrollTop=chat.scrollHeight;
  try{
    let answer="";
    if(typeof TUTOR_SETTINGS!=="undefined" && TUTOR_SETTINGS.enabled && TUTOR_SETTINGS.endpoint){
      const res=await fetch(TUTOR_SETTINGS.endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:userText,lesson:{title:l.title,area:l.area,goal:l.goal,theory:l.theory,detail:l.detail,map:l.map}})});
      if(!res.ok) throw new Error("Errore Tutor AI");
      const data=await res.json();
      answer=data.answer||"Non sono riuscito a generare una risposta.";
    }else{
      answer=localTutorAnswer(userText,l);
    }
    const loading=$("tutorLoading");
    if(loading) loading.remove();
    chat.innerHTML+=`<div class="tutor-msg tutor-ai"><b>Tutor:</b> ${escapeHtml(answer)}</div>`;
  }catch(e){
    console.warn(e);
    const loading=$("tutorLoading");
    if(loading) loading.remove();
    chat.innerHTML+=`<div class="tutor-msg tutor-ai"><b>Tutor:</b> Il tutor online non risponde. Posso comunque aiutarti con la spiegazione locale del capitolo.\n\n${escapeHtml(localTutorAnswer(userText,l))}</div>`;
  }
  chat.scrollTop=chat.scrollHeight;
}
function localTutorAnswer(userText,l){
  const q=String(userText||"").toLowerCase();
  const teoria=(l.theory||[]).slice(0,5).join("\n- ");
  const dettagli=(l.detail||[]).slice(0,4).join("\n- ");
  const mappa=(l.map||[]).join(" → ");
  if(q.includes("orale")||q.includes("domande")){
    const oral=(l.oral||[]).slice(0,6).map((x,i)=>`${i+1}. ${x[0]}`).join("\n");
    return `Alleniamoci all'orale sul capitolo: ${l.title}.\n\n${oral}\n\nMetodo per rispondere:\n1. Parti dalla parola chiave.\n2. Dai una definizione breve.\n3. Fai un esempio pratico.\n4. Non imparare frasi a memoria: spiega con parole tue.`;
  }
  if(q.includes("esempio")){
    return `Esempio pratico sul capitolo: ${l.title}.\n\nPensa a una situazione reale: in una scuola agraria puoi usare questi concetti per organizzare dati su piante, irrigazione, temperature, file di relazione, grafici o presentazioni.\n\nMappa da seguire: ${mappa}\n\nFrase per l'orale: "${l.title} riguarda ${l.area}. Serve a capire e usare correttamente questo argomento nella pratica."`;
  }
  if(q.includes("semplice")||q.includes("zero")||q.includes("capito")){
    return `Te lo spiego semplice.\n\nCapitolo: ${l.title}\n\n- ${teoria}\n\nTrucco: non cercare di ricordare tutto insieme. Ricorda prima la mappa:\n${mappa}\n\nPoi prova a dire: "È..., serve a..., per esempio..."`;
  }
  return `Ripasso guidato del capitolo: ${l.title}\n\nSpiegazione base:\n- ${teoria}\n\nSpiegazione in più:\n- ${dettagli}\n\nMappa mentale:\n${mappa}\n\nNon ti do risposte ai quiz, però posso spiegarti ogni termine con parole più semplici.`;
}
